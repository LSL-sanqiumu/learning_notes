...
什么是程序？编写的代码都是为了实现特定的功能，实现这些功能所需要的代码就是一条条指令，一条条指令组合成了一个程序，也就是说，程序就是为了完成特定的任务，用编程语言写出来的一条条指令的集合体。
...
学习目的：知道类的实用，类是干什么的、以及方法的作用。

## 包装类（Wrapper）：

包装类：针对八种基本数据类型定义的相应的引用类型，Byte、Short、Integer、Long、Float、Double、Character、Boolean。
...
- 内部定义final char value[]用于存储字符串数据；
...
- 例如
  - 声明了两个注解MyAnnotation和MyAnnotations；
