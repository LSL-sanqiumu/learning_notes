...
继承：一个类可以作为子类来继承父类，使得子类有父类的数据或行为方法。继承可以方便功能拓展、提高代码的复用性，也为多态的使用提供了前提，继承是对代码的复用。（继承的是父类的结构（属性和方法））。继承特性：一个类可以为多个子类继承，一个子类只能有一个父类；可以多层继承。
...
（我的理解：为了抽取类的更加通用的特性而抽象，是对一类事物共同特征的高度总结概括）。
