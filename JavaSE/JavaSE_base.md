...
浮点数精确位：
- double：小数点后最多会保留十七位有效数字，最少一位，小数点后有效位数会随着个位十位等位数增加而减少；
- float：小数点后最多保留八位有效数字，最少一位，小数点后有效位数会随着个位十位等位数增加而减少；
- 对运算结果是小数的运算要小心。（double result = 1.0 - 0.9 = 0.09999999999999998;`出现精度丢失问题）
...
- 同一个类中允许一个以上的同名方法，只要他们的**参数个数或者参数类型、参数顺序**不同就能构造重载，方法返回值和访问修饰符可以不同，可以有static关键字；
- 可变形参个数方法  例如： **public void show(数据类型  ...  变量名){ }**          
  ...
  - 可变形参最多只能有一个并且声明在末尾，例如**public void show(int i,  String ... strs) {}**。
...
- 局部内部类是一个类，但不能使用权限修饰符；可以使用final，可以使用外部类的结构；
...
new AbstractClass("陆拾陆"){
    @Override
    void introduce() {
        System.out.println("我是一个匿名的抽象子类的匿名");
    }
};
```
...
接口技术：用来描述类具有什么功能，但并不给出每个功能的具体实现（**接口是抽象方法的集合**）；接口不是类，而是对类的一组需求描述，这些类要遵从接口描述的统一格式进行定义（也就是接口实现类必须提供并重写接口定义的抽象方法）。  
