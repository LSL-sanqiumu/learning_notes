...
- **内容**
  ...
  - JDK8同时还推出了函数式接口（使用@FunctionInterface注解描述）设计。
- **特性**
...
如上图的RntimeException，下面代表的的异常分别是出现异常的运算条件的异常（例如除以0）、类类型转换错误的异常、方法形参传递错误的异常、Java 环境或 Java 应用程序没有处于请求操作所要求的适当状态下的异常、数组等排序索引超出范围的异常、找不到元素的异常、空指针异常。
...
3.【注意】catch里面的类型都没有父子关系的时候，顺序无所谓；如果catch中异常类型满足子父类关系的时候，子类的要写在父类之前
...
写在方法声明处，指明此方法执行时可能会抛出的异常类型，其作用是当方法体执行时出现异常时，生成一个对应异常类的对象，此异常对象满足throws后指定的异常类型时，就会被抛出，异常抛出后后续代码就不再执行！
【注意】    throws 只是将异常抛给方法的调用者，并没处理掉异常
    	   try 结构才是真正处理掉了异常
```
...
- super关键字是一个指示编译器调用超类方法的特殊关键字，意为**当前对象的父类**（super用来区分子类和父类，和this关键字有些相像之处）。
- super可以用来区分重写与被重写的方法，区分父类、子类都显式声明了的属性：
...
- 调用父类构造器：
  ...
  - 用this时是先在当前类里找，如果没找到也会去父类里找，有点就近原则的意思。



## static

- 因为在编写类时并没有产生实质上的对象，当通过new关键字时才产生了对象并进行了内存空间分配；为了实现特定的属性或方法只存在一份内存里，就发展出了static关键字。
