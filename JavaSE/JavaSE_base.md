...
- 同一个类中允许一个以上的同名方法，只要他们的**参数个数或者参数类型、参数顺序**不同就能构造重载，重载方法的方法返回值和访问修饰符可以不同，也可以有final、static关键字。
...
- 对于只使用一次的某个类的对象或某接口的实现对象，使用匿名内部类就简化了开发。
