...
Java中有两种数据类型：基本数据类型、引用类型（类、接口和数组）。8种基本数据类型如下：
...
- 初始化：(只能是如下静态或动态初始化，一经初始化数组长度就已经确定)
...
- 后面的方框是代表一维数组；
- 二位数组从底层来说就是一个一维数组里的值还是一维数组。

数据结构：

- 1.数据与数据之间的逻辑关系：集合、一对一、一对多、多对一
- 2.数据的存储结构：顺序表（比如数组）、链表、栈、队列
- 树形结构：二叉树、图形结构：
...
冒泡排序：从前往后比较相邻元素大小，发现逆序则交换（每一次排序都把最大的排好）
...
类的访问控制符只能是空或者 public，空的在同一包下可以引用，public的可以在同一工程下引用（通过import机制）。
...
Java中允许一个类定义在一个类的内部，这个定义在类内部的类称为内部类（inner class），相对的外面的则称为外部类。内部类作为类的结构时称为成员内部类，类的内部还有其他的结构，在这些结构内声明的类则称为局部内部类。
...
封装就是把抽象出的数据和对数据操作的方法封装在一起，数据被保护在内部，程序的其他部分只有通过被授权的操作，才能对数据进行操作。
...
![](../../../../../../LSLBlogs/blogs_content/JavaSE/img/exception.png)
...
- 含义**为当前对象的父类**（super用来区分子类和父类，和this关键字有些相像之处）；
...
- 表示最终的，用来修饰类、方法、变量、对象；
- 使用：
  - 修饰类：修饰后的类不能被继承，例如String类、System类、StringBuffer类；
  - 修饰方法：修饰后的方法不能被重写，比如Object类的getClass()	；
  - 修饰变量：修饰的“变量”就成为一个常量，表示这个变量只能被赋值一次（如果是引用类型的是不是就是变量地址是确定的）；
