...
| 函数                                        | 功能                                        |
| ------------------------------------------- | ------------------------------------------- |
| date_add(date,interval expr 日期或时间类型) | 一个时间或日期的值加上时间间隔expr后的值    |
| datediff(date1,date2)                       | date1与date2之间间隔的天数（date1 - date2） |
...
- 主键索引（PRIMARY KEY）：是针对表的主键创建的索引，默认自动创建且每张表只能有一个。
- 唯一索引（UNIQUE KEY）：可以存在多个唯一索引，多个列都可以标识为唯一索引，是为了避免重复的列出现。
- 常规索引（key、index）：默认的索引，使用key关键字来设置，用来快速定位特定数据，可以有多个。
- 全文索引（fulltext）：特定的引擎下才有(myisam)，全文索引查找的是文本中的关键字，而不是比较索引中的值，可以有多个。
- ![](img/5.优化.png)

Innodb中，根据索引的存储形式来分类，可分为：

1. 聚集索引：将数据存储和索引放在一块，索引结构的叶子节点保存了行数据。（一张表中必须用且只有一个）
   - 如果存在主键，那么主键索引就是聚集索引。
