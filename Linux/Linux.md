...
3. `ls -lh`（简化后法是`ll`，可得到同样效果）：以更人性化的方式显示当前目录下的内容。
...
- `chmod 751 文件或目录`：给u-所有者7、给g-所有组5、给其它组-1的权限；
...
1. `crontab -e`，进入任务调度设置界面，`i`切换，写任务调度指令（例如 `*/10 * * * * date >> /home/mydate.txt`），然后`:wq`保存并退出。
2. 执行脚本：
...
3. 数据库备份：`mysqldump -u root -p密码 数据库 >> /home/db.bak`；
