...
7. `whoami`：查询当前登陆的用户（不是切换到的用户）。
8. `who am i`：最开始是用哪个登陆的就显示哪个，切换用户后再登录也是刚刚开始时的。
9. `passwd 用户名`：可以为指定用户更改密码，如果不加用户名则是为当前登陆的用户更改密码。
...
1. `cat [选项] 要查看的文件`：选项为`-n`是显示行号，只能查看文件，一般使用`cat -n 文件 | more`（管道命令`| more`，限制显示，按回车往下翻一行）。
2. `more 文件`：查看文件。
3. `less 文件`：查看文件。
4. `head 文件`：默认查看文件内容的头十行；`head -n 5 文件`用于指定显示头几行。
5. `tail 文件`：默认查看文件尾十行的内容；`tail -n 5 文件`；`tail -f 文件`（实时追踪文件的所有更新，会占有屏幕，实时显示内容）。
...
1. `gzip 文件`：只能压缩文件，不会保留原始文件，执行后获得.gz结尾的压缩包。
2. `gunzip 文件.gz`：解压.gz结尾的文件，不会保留原始压缩包。
