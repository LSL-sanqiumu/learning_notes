...
C:\Users\Scholar\AppData\Local\Packages\CanonicalGroupLimited.Ubuntu20.04onWindows_79rhkp1fndgsc
...
安装虚拟机：VMware Workstation 16.1.0 Pro，激活码如下；
...
- 新建虚拟机：
  ...
  7. 完成。
- 引入ISO文件：选中虚拟机右键选择设置，然后选中CD/DVD中的使用ISO映像文件，找到下载的ISO文件；
- 启动此虚拟机，然后双击出现的黑屏，选择安装install，进入安装界面再按提示按回车；
- 然后出现选择语言的，找到中文，选择中文，继续；
- 进入安全信息摘要界面，要等待不要点鼠标，否则容易卡死；
...
- 点击完成，然后再继续等待检查软件依赖关系（等待四分钟左右不要动鼠标，否则卡死哦）；
- 然后再选择安装位置：进入选择我要配置分区，点击完成后，进入手动分区界面
  ...
  - 点击完成，接受修改。
- KDUMP在实际工作中开启，学习时可不开启；
- 然后是网络和主机，打开以太网，可以改一下主机名（不能有空格）；
- 最后一个是安全策略，一般可以关闭；
- 安装，root用户和设置密码（实际工作中建议复杂一点），再建一个普通用户；
- 等待安装完成，会出现重启，点击重启，然后进入系统，中间要接受一个协议，然后完成配置，进入系统；
- 用户名和密码登录，安装成功。
- 进入后配置：语言选汉语、输入选拼音、隐私关闭、跳过、开始使用。
- 右上角关机。
- 释放鼠标（Ctrl + Alt）。
...
- 新建会话：名称可随意，主机是要登录的远程系统的IP，协议SSH，端口22；然后点击确定；
- 双击会话，学习中可选择一次性接收，工作中建议选择接受并保存，然后输入用户名、密码；

Xftp：Windows平台的功能强大的SFTP、FTP文件传输软件，使Windows用户可以在UNIX/Linux和Windows之间安全地传输文件。

安装完成好使用Xftp：

- 新建会话：称可随意，主机是要登录的远程系统的IP（192.168.137.128），协议SFTP，端口22；然后点击确定；
- 双击会话，学习中可选择一次性接收，工作中建议选择接受并保存，然后输入用户名、密码。
...
- 命令行模式：输入`:`进入，在这个模式中，可以提供相关指令来完成读取、存盘、替换、离开vim、显示行号等；
...
- yy：拷贝当前行，5yy：拷贝当前行向下的5行（从当前行开始复制）；（一般模式）
- p：粘贴；（一般模式）
- dd：删除当前行，5dd：删除当前行向下的5行（从当前行开始删除）；（一般模式）
- u：撤销操作；（一般模式）
- gg：跳到首行；G：调到末行；（一般模式）
- 输入要到达的行数然后再`shift+g` ：调到指定行；（一般模式）
  - 可以先显示行号：`:set nu`，然后退回到一般模式，输入行号这个数，再输入`shift+g`即可定位到指定行数。
- 设置文件行号：`set nu`，取消文件行号：`set nonu`；（命令行模式）
- `/查找内容`：查找关键字， 然后按n可以查找下一个；（命令行模式）



## 用户管理

- `useradd 用户名`：该命令原来添加用户，添加后自动创建用户家目录：/home/用户名；
- `useradd -d /home/test usertest`：可自定义用户家目录名；
- `userdel 用户名`：删除用户但不删除用户的家目录；
- `userdel -r 用户名`：删除用户并且删除用户的家目录；
- `id 用户名`：查询有无此用户；
- `su - 用户名`：用于切换用户，从权限高的用户切换到权限低的用户不需要密码，返回原来的用户使用exit或logout命令；
- `who am i`、`whoami`：查询当前登陆的用户（不是切换到的用户）；
- `passwd 用户名`：可以为指定用户更改密码，如果不加用户名则是为当前登陆的用户更改密码。
...
登录时尽量少用root用户，登录普通用户后可以再使用`su - root`命令切换到root用户。

在普通用户下切换到root用户，再用`logout`命令可退出root用户，返回到普通用户。

（注意：logout在图形界面的终端下运行无效，在运行级别3下才有效）。
...
- CentOS7后运行级别：
  ...
  - graphical.target是图形界面级别，multi-user.target是运行级别3。
- 如果重启，会是切换后的运行级别。
...
1.重启进入该页面，按上下光标停止倒计时，再按e键进入下面的页面按下键移动光标，找到linux16开头的：如下

![](img/e.png)

2.在linux16开头的后面，即UTF-8后面，先空格再输入`init=/bin/sh`；

3.然后按Ctrl + X启动，进入单用户模式；

进入单用户模式后，在光标闪烁位置输入`mount -o remount,rw / `（逗号两边不要有空格，rw后空格再加`/`）；如下图：

![](img/单用户模式.png)

4.按回车，然后在新一行输入`passwd`，输入后再按回车，输入密码回车，再次输入确认密码后，按回车，密码设置完成；

![](img/passwd.png)

5.再输入`touch /.autorelabel`，回车；

6.再在新的一行输入`exec /sbin/init`，按回车后等待系统自动修改密码（可能时间有点长，耐心等待），完成后，系统或重启，新密码生效。
...
- `pwd`：显示当前工作目录的绝对路径；
- `ls [选项] [目录或文文件]`：选项是`-a`，则显示当前目录的所有文件和目录（包括隐藏的）；选项是`-l`则以列表的形式显示没有隐藏的；
- `ls -lh`：以更人性化的方式显示当前目录下内容；
...
- `mkdir [选项] 要创建的目录名`：不用选项则直接创建单个目录，选项`-p`则是创建多级目录；
- `touch 文件名称`：创建空文件；
...
- `rm [选项] 要删除的文件或目录`：选项为`-r`则是递归删除整个文件夹，`-f`则是强制删除不再提示；`rm hello.txt`；
- `rmdir [选项] 要删除的空目录`：只能删除空目录，删除非空目录要使用指令：`rm -rf 要删除的目录`；

### 重命名、移动：

- `mv xx xx`：重命名文件、目录（`mv 旧文件/目录 新文件/新目录`），移动文件、目录（`mv 文件或目录地址 目标目录`）；

### 输出内容到控制台：

- `echo xx`：`echo $PATH,$HOSTNAME`（输出环境变量），`echo hello`（输出hello）；
- echo和`>`、`>>`结合使用，前者是输出重定向（覆盖），后者是追加；想·想·
- `echo 'hello' > hello.txt`将hello重定向到该文件，会把原来内容覆盖； 
- `cal >> xx`：追加日期到文件
...
- `cat [选项] 要查看的文件`：选项为`-n`是显示行号，只能查看文件，一般使用`cat -n 文件 | more`（管道命令`| more`，限制显示，按回车往下翻一行）； 
- `more 文件`：查看文件；
- `less 文件`：查看文件；
- `head 文件`：默认查看文件内容的头十行；`head -n 5 文件`用于指定显示头几行；
- `tail 文件`：默认查看文件尾十行的内容；`tail -n 5 文件`；`tail -f 文件`（实时追踪文件的所有更新，会占有屏幕，实时显示内容）；
...
- `ln -s [原文件或目录] 软链接名`：在当前目录下创建文件或目录的软链接，删除使用rm；
- 软连接类似Windows里的快捷方式；

### 查看历史命令：

- `history`：查看已经执行过的命令；
- `history 数字`：查看最近的多少条指令；
- `!指令前的数字`：使用history查看到历史指令后使用该指令可以执行指定的历史指令；
...
- 最终结果一样：`cat /home/hello.txt | grep -n "yes"`；`grep -n "yes" /home/hello.txt`；
...
- `unzip [选项] xxx.zip`：解压缩文件，常用的选项是`-d`，即指定解压后文件的存放目录；
  - `unzip -d /home xxx.zip`：解压到/home目录。

gzip，只能压缩文件：

- `gzip 文件`：只能压缩文件，不会保留原始文件，执行后获得.gz结尾的压缩包；
- `gunzip 文件.gz`：解压.gz结尾的文件，不会保留原始压缩包。
...
- 打包压缩文件或目录：
  - `tar -zcvf xxx.tar.gz xxx文件 xxx文件`：可打包压缩多个文件并产生.tar.gz打包压缩文件；
  - `tar -zcvf xxx.tar.gz xxx目录`：打包并压缩目录。
- 解压：
  - `tar -zxvf xxx.tar.gz`：解包到当前目录；
...
- `-`：在第0位的表示的是普通文件；
- 第2、3、4点表示的位用`rwx`和`-`表示，此时`-`表示没有该权限；关于rwx权限：
- ![](img/grouprwx.png)
...
- r是4、w是2、x是1，组合成1-7表示7个权限搭配，rwx = 4 + 2 + 1 = 7；
- `chmod 751 文件或目录`：给u所有者7、给g所有组5、给其它组1的权限；
...
Linux后台运行着一个管理任务调度的程序，可以利用其执行特定时间段的命令，任务调度就是指某个时间执行的特定命令或程序。
...
- `crontab -e`，进入任务调度设置界面，`i`切换，写任务调度指令（例如 `*/10 * * * * date >> /home/mydate.txt`），然后`:wq`保存并退出；
- 执行脚本：
  - 编写脚本，例如`vim /home/my.sh`进入脚本编写可执行指令（例如`date >> /home/mycal.txt`和 `cal >> /home/mycal.txt`）；
  - 給脚本增加执行权限（主要給root用户），`chomd u+x /home/my.sh`；
  - `crontab -e`，增加`*/1 * * * * /home/my.sh`。
- 数据库备份：`mysqldump -u root -p密码 数据库 >> /home/db.bak`；
...
- 执行完作业后作业会被移出；
- `at [选项] [时间]`，执行此命令后进入任务编辑，按Ctrl + D退出作业编辑（连按两次（Ctrl + DD））；
- `atq`：查询要执行的工作任务；
...
Linux每个分区都是组成整个文件系统的一部分，Linux的整个文件系统中包含了一整套的文件和目录；
...
1. 对新硬盘分区：
   - `fdisk /dev/硬盘名`：硬盘名就像上面的sda，新硬盘一般是sdb，执行后进入分区操作；
...
service服务，运行在后台的进程，通常都会监听端口，等待其它程序的请求，例如mysqld、sshd、防火墙等，因此又称为守护进程。

- `ls -l /etc/init.d`：查看service指令管理的服务；`setup`可以查看所有的服务，带`*`的是会开启重启的；
